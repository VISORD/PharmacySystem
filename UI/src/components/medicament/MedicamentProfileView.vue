...
async function show() {
    await router.push({
        ...
        query: { medicamentId: medicament.view.medicamentId }
    })
...
<template>
    <Dialog
        ...
        @show="show()"
        @hide="hide()"
        :header="medicament.view.profile ? `Medicament info: ${medicament.view.profile.name}` : 'Medicament info'"
